...
//   int a[][3]={{1,2,3},{4,5,6}}}; cout<<"a11="<<a[1][1]<<endl;
   int itmp; bool check;
    fex1=argv[1]; fex2=fex1;   string infi(argv[2]);  int pos=infi.find_last_of('/');
    string odir=infi.substr(0,pos+1);
   for(int i=1;;i++) {stringstream fn;// sprintf(fn,"%i",i);
       fn<<odir<<i;
	   ifstream checkfi(fn.str().c_str());
	   if(!checkfi.good()) { ifn=i-1; checkfi.close(); break;}
	   checkfi.close();
   }
 if ((argc>1)&&(argv[1][0]=='.'))  {
	 Problem.stat(ifn); return 0; }
     else if ((argc>1)&&(argv[1][0]=='c'))  {
	 chekxi(odir); return 0; }
	 else{
     cout.precision(3);
...
//      suxx=horse.label(); 
		...
		analis.setx00(xi0);
           for(int i=0;i<nmet;i++) xx[i]=xinit1[i];
                int ifn0=250002; Problem.write(tf,odir,ifn0,xi0,suxx,0);
