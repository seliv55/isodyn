	...
	nrdens(nrdensin), beta(betain), meth(1), nstiff(nstiffin), icont(icontin)
{
	...
	if ((iout < 0) || (iout > 2)) {
		std::cout << "Wrong input, iout = " << iout << std::endl;
		throw -1;
	}
	...
	if (beta == 0.0)
		...
		std::cout << "Curious input for beta : beta = " << beta << std::endl;
		throw -1;
	}
	...
	if (nrdens > n) {
		std::cout << "Curious input, nrdens = " << nrdens << std::endl;
		throw -1;
	}
	else if (nrdens != 0) {
		...
		if (nrdens == n) {
			if (icont) std::cout << "Warning : when nrdens = n there is no need " <<
				"allocating memory for icont\r\n";
		}
		else {
			if (iout < 2) std::cout << "Warning : put iout = 2 for dense output\r\n";
...
int NonStiffIntegratorT::CoreIntegrator()
{
	...
	if (iout != 0) {
		...
		if (irtrn < 0) {
			std::cout << "Exit of dopri5 at x = " << x << std::endl;
			return 2;
		}
	}
	...
	while (true) {
		if (nstep > nmax) {
			std::cout << "Exit of dopri5 at x = " << x << ", more than nmax = "
				<< nmax << " are needed." << std::endl;
			hold = h;
			return -2;
		}
		if (0.1*fabs(h) <= fabs(x)*uround) {
			std::cout << "Exit of dopri5 at x = " << x << ", step size too small h = "
				<< h << std::endl;
			hold = h;
			return -3;
		}
		...
		// step accepted
			...
			if (!(naccpt % nstiff) || (iasti > 0)) {
				...
				if (hlamb > 3.25) {
					...
					iasti++;
					if (iasti == 15) {
			std::cout << "The problem seems to become stiff at x = " << x << std::endl;
						hold = h;
						return -4;
					}
				}
...
double NonStiffIntegratorT::ContinuousOutput(unsigned i)
{
	...
	if (ii == UINT_MAX) {
		std::cout << "No dense output available for %uth component" << i << std::endl;
		return 0.0;
	}
